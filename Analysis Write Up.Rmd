...
model_data <- df %>%
  ...
  mutate(experimental_group = factor(experimental_group, c("upperclass","freshman", "prolific"))) %>%
...
model_data %>%
  ...
  scale_color_viridis(discrete = TRUE, option = "C",begin = .2, end = .8) + # added here for color, also needed to add aes groupings
...
juice(pca_prep) %>%
  ...
  scale_color_viridis(discrete = TRUE, option = "C", begin = .2, end = .8) +
  theme_minimal() +
  theme(legend.position="bottom") +
...
legend("topright", 
     legend = c("Chromatic PD" , "Common PD" , "Bridge Chords" , "Non PDs"), 
     col = c(
           "#440154FF",  # Purple
              "#39568CFF",  # Blue
    "#FDE725FF",  # Yellow 
...
```{r}
# Attempt to Make HCA Better Color
